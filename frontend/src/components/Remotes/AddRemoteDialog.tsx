...
import { createRemote, updateRemote, verifyRemoteConfig } from "@/api";
      ...
      <DialogContent className="sm:max-w-[600px] overflow-y-auto max-h-[85vh]">
        ...
        <div className="grid gap-6 py-4">
          <div className="grid gap-2">
            ...
            <Input id="name" value={name} onChange={(e: any) => setName(e.target.value)} placeholder="e.g. MyGoogleDrive" />
          </div>
          
          <div className="grid gap-2">
            <Label htmlFor="type">Provider Type</Label>
            <Select value={type} onValueChange={(v: any) => setType(v as string)}>
              <SelectTrigger>
            ...
            <div className="space-y-4 border-t pt-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="grid gap-2">
                  ...
                  <Input id="clientId" value={clientId} onChange={(e: any) => setClientId(e.target.value)} placeholder="Custom Client ID" />
                </div>
                <div className="grid gap-2">
                  <Label htmlFor="clientSecret">Client Secret (Optional)</Label>
                  <Input id="clientSecret" type="password" value={clientSecret} onChange={(e: any) => setClientSecret(e.target.value)} placeholder="Custom Secret" />
                </div>
              </div>
              
              <div className="grid gap-2">
                <Label>Rclone Token (Required)</Label>
                <Input placeholder='{"access_token":"...","token_type":"Bearer",...}' value={token} onChange={(e: any) => setToken(e.target.value)} />
              </div>
            </div>
          )}
          ...
          {isBasicAuthType && (
            <div className="space-y-4 border-t pt-4">
              <div className="grid gap-2">
                ...
                <Input id="host" value={host} onChange={(e: any) => setHost(e.target.value)} placeholder={type === "webdav" ? "https://nextcloud.example.com/remote.php/webdav/" : "sftp.example.com"} />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div className="grid gap-2">
                  <Label htmlFor="user">Username</Label>
                  <Input id="user" value={user} onChange={(e: any) => setUser(e.target.value)} placeholder="admin" />
                </div>
                <div className="grid gap-2">
                  <Label htmlFor="pass">Password</Label>
                  <Input id="pass" type="password" value={password} onChange={(e: any) => setPassword(e.target.value)} placeholder={editItem ? "(Leave blank to keep unchanged)" : "••••••••"} />
                </div>
              </div>
            </div>
          )}
