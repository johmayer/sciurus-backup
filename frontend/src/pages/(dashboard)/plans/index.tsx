...
import { getPlans, deletePlan, runPlanNow, updatePlan, cancelPlan } from "@/api";
...
const handleToggleEnabled = async (id: string, currentEnabled: boolean) => {
    try {
      ...
      const payload = {
          ...
          sourceId: planToUpdate.sourceId || planToUpdate.source_id || (planToUpdate.source ? planToUpdate.source.id : ""),
          remoteId: planToUpdate.remoteId || planToUpdate.remote_id || (planToUpdate.remote ? planToUpdate.remote.id : ""),
          schedule: planToUpdate.schedule,
          encrypt: planToUpdate.encrypt,
          password: "",
          enabled: !currentEnabled,
          remoteFolderPath: planToUpdate.remoteFolderPath || planToUpdate.remote_folder_path || "",
          backupPrefix: planToUpdate.backupPrefix || planToUpdate.backup_prefix || "backup"
      };
  ...
  return (
    <div className="flex flex-col gap-4">
              ...
              {plans.map((plan) => {
                ...
                <React.Fragment key={plan.id}>
                  <TableRow className="cursor-pointer hover:bg-muted/50 transition-colors" onClick={() => openEdit(plan)}>
                    ...
                    <TableCell onClick={(e: any) => e.stopPropagation()}><Switch checked={plan.enabled} onCheckedChange={() => handleToggleEnabled(plan.id, plan.enabled)} /></TableCell>
                    <TableCell>
                    ...
                    <TableCell className="text-right">
                      <div className="flex items-center justify-end space-x-2">
                        <Button variant="outline" size="sm" onClick={(e: any) => { e.stopPropagation(); setRestoreItem(plan); }}>
                          <DownloadCloud /> Restore
                        </Button>
                        {isRunning ? (
                          <Button variant="outline" size="sm" onClick={(e: any) => { e.stopPropagation(); handleCancel(plan.id); }}>
                            <StopCircle className="text-destructive" /> Abort
                          </Button>
                        ) : (
                          <Button variant="outline" size="sm" onClick={(e: any) => { e.stopPropagation(); handleRunNow(plan.id); }}>
                            <Play /> Run Now
                          </Button>
                        )}
                        <Button variant="ghost" size="sm" onClick={(e: any) => { e.stopPropagation(); openEdit(plan); }}>
                          Edit
                        </Button>
                        <Button variant="ghost" size="sm" onClick={(e: any) => { e.stopPropagation(); setDeleteId(plan.id); }}>
      ...
      <RestorePlanDialog open={!!restoreItem} onOpenChange={(open: any) => !open && setRestoreItem(null)} plan={restoreItem} />
