...
import { Source, Remote } from "@prisma/client";
import { Card, CardContent, CardDescription, CardHeader, CardTitle, CardFooter } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import { CheckCircle2, XCircle, Loader2, Edit, Trash2 } from "lucide-react";
import { validateSourcePath, validateRemoteById, markConfigValidated, deleteSource, deleteRemote } from "@/app/actions";
...
export default function ValidateClient({ 
  ...
  useEffect(() => {
    initialSources.forEach(s => checkSource(s));
    initialRemotes.forEach(r => checkRemote(r));
  }, [initialSources, initialRemotes]);

  const allDone = initialSources.every(s => sourceStatus[s.id]?.valid) && 
                  initialRemotes.every(r => remoteStatus[r.id]?.valid);
                  
  const anyLoading = initialSources.some(s => sourceStatus[s.id]?.loading) || 
                     initialRemotes.some(r => remoteStatus[r.id]?.loading);
  ...
  return (
    <Card className="w-full max-w-2xl">
      ...
      <ConfirmDeleteDialog 
        open={!!deleteRemoteId} 
        onOpenChange={(open) => !open && setDeleteRemoteId(null)}
