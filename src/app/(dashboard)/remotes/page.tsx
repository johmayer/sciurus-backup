...
import { getRemotes, deleteRemote } from "@/app/actions";
              ...
              {remotes.map((remote) => (
                <TableRow key={remote.id} className="cursor-pointer hover:bg-muted/50 transition-colors" onClick={() => openEdit(remote)}>
                  ...
                  <TableCell className="text-right space-x-2">
                    <Button variant="ghost" size="sm" onClick={(e) => { e.stopPropagation(); openEdit(remote); }}>
                      Edit
                    </Button>
                    <Button variant="ghost" size="sm" onClick={(e) => { e.stopPropagation(); setDeleteId(remote.id); }}>
